    ...
    def sample_images(self, epoch,batch_size):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig("images%d/mnist_%d.png" %(batch_size, epoch))
        plt.close()
...
if __name__ == '__main__':
    wgan = WGAN()
    begin8=time.time()
    wgan.train(epochs=4000, batch_size=8, sample_interval=50)
    end8=time.time()
    begin16=time.time()
    wgan.train(epochs=4000, batch_size=16, sample_interval=50)
    end16=time.time()
    begin32=time.time()
    wgan.train(epochs=4000, batch_size=32, sample_interval=50)
    end32=time.time()
    begin50=time.time()
    wgan.train(epochs=4000, batch_size=50, sample_interval=50)
    end50=time.time()
    begin64=time.time()
    wgan.train(epochs=4000, batch_size=64, sample_interval=50)
    end64=time.time()
    begin100= time.time()
    wgan.train(epochs=4000, batch_size=100, sample_interval=50)
    end100=time.time()
    begin128=time.time()
    wgan.train(epochs=4000, batch_size=128, sample_interval=50)
    end128=time.time()
    begin150=time.time()
    wgan.train(epochs=4000, batch_size=150, sample_interval=50)
    end150=time.time()
    begin200=time.time()
    wgan.train(epochs=4000, batch_size=200, sample_interval=50)
    end200=time.time()
    begin250=time.time()
    wgan.train(epochs=4000, batch_size=250, sample_interval=50)
    end250=time.time()
    begin256=time.time()
    wgan.train(epochs=4000, batch_size=256, sample_interval=50)
    end256=time.time()
    begin512= time.time()
    wgan.train(epochs=4000, batch_size=512, sample_interval=50)
    end512=time.time()
    begin1024= time.time()
    wgan.train(epochs=4000, batch_size=1024, sample_interval=50)
    end1024=time.time()
